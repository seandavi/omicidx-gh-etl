...
@task(retries=1)
def task_get_needed_urls(replace: bool = False):
    return get_needed_urls(replace=replace)


@task(retries=1)
def task_pubmed_url_to_parquet_file(url: UPath):
    pubmed_url_to_parquet_file(url)
...
def etl_pubmeds(replace: bool = False):
    needed_urls = task_get_needed_urls(replace=replace)
    ...
    for index, url in enumerate(needed_urls):
        ...
        task_pubmed_url_to_parquet_file(url)  # type: ignore
